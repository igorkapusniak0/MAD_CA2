...
import ie.setu.mobileappdevassignment.adapter.DiscoverTabPagerAdapter
...
class DiscoverActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = DiscoverTabPagerAdapter(this)
        viewPager.adapter = adapter

        TabLayoutMediator(tabLayout, viewPager) { tab, position ->
            tab.text = when (position) {
                0 -> getString(R.string.public_collections)
                1 -> getString(R.string.public_sets)
                else -> null
            }
        }.attach()
